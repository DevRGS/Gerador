...
for modulo, pacote in dependencias.items():
    try:
        ...
        # Adiciona tratamento de erro para a instalação
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", pacote])
            importlib.invalidate_caches() # Limpa o cache após instalar
            importlib.import_module(modulo) # Tenta importar novamente
        except subprocess.CalledProcessError as e:
             print(f"ERRO: Falha ao instalar o pacote '{pacote}'. Verifique sua conexão e pip. Erro: {e}")
             # Considerar sair do script se uma dependência crítica falhar
             # sys.exit(1)
        except ImportError:
            print(f"ERRO: Falha ao importar '{modulo}' mesmo após tentar a instalação.")
            # sys.exit(1)
...
# from pptx.util import Inches # Se precisar definir tamanhos
...
def baixar_arquivo_if_needed(nome_arquivo, url):
    ...
    if not os.path.exists(nome_arquivo):
        ...
        try:
            r = requests.get(url, timeout=20) # Timeout maior
            r.raise_for_status() # Verifica erros HTTP
            with open(nome_arquivo, "wb") as f:
                f.write(r.content)
            print(f"{nome_arquivo} baixado com sucesso.")
        except requests.exceptions.RequestException as e:
            showerror("Erro de Download", f"Não foi possível baixar {nome_arquivo} de {url}\nErro: {e}")
            # Você pode querer lançar uma exceção aqui para parar, ou apenas avisar
            # raise e # Para parar a execução
...
def carregar_config(nome_closer_var, celular_closer_var, email_closer_var):
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            print(f"Aviso: Arquivo {CONFIG_FILE} corrompido ou inválido.")
...
# ---------------------------------------------------------
# Dados de Planos e Tabelas de Preço - ATUALIZADO COM NOVA LÓGICA
# ---------------------------------------------------------
...
PLAN_INFO = {
    ...
    # Manter Bling/Autoatendimento/Em Branco como na versão anterior ou ajustar conforme necessário
    "Autoatendimento": {
    ...
    "Em Branco": {
        "base_mensal": 0.0, "min_pdv": 0, "min_users": 0, "max_extra_users": 999, "max_extra_pdvs": 99,
        "mandatory": [],
        "allowed_optionals": [m for m in precos_mensais.keys()] # Permite todos os opcionais? Ajustar.
    }
}

# Módulos SEM DESCONTO (Revisar esta lista)
SEM_DESCONTO = {
    "TEF", "Terminais Autoatendimento", "Smart TEF", # Opcionais com preço fixo
    "Delivery Direto Profissional", "Delivery Direto VIP", # Opcionais com preço fixo
    "Programa de Fidelidade", "Integração Tap", "Integração API", # Opcionais com preço fixo
    "Business Intelligence (BI)", "Backup Realtime", # Opcionais com preço fixo
    # Adicionar outros se necessário (Ex: Smart Menu? Hub Delivery?)
    "Hub de Delivery", "Smart Menu",
}

# Preços MENSAIS dos Módulos/Extras (Revisado)
precos_mensais = {
    # Módulos que podem ser Opcionais (preço mensal)
    ...
    "Backup Realtime": 199.90, # Manter o maior valor ou diferenciar?
    "Business Intelligence (BI)": 199.00, # Manter o maior valor ou diferenciar?
    "Programa de Fidelidade": 299.90, "Integração Tap": 299.00, "Integração API": 299.00,

    # Módulos sempre fixos (preço zero aqui, incluídos na base)
    ...
    # Controlados por Spinbox (preço zero aqui, custo calculado à parte)
...
PRECO_EXTRA_PDV_BLING = 40.00 # Definir preço extra para Bling se aplicável

# ---------------------------------------------------------
# Função utilitária para substituir placeholders no Slide - VERSÃO SIMPLES (do backup)
# ---------------------------------------------------------
def substituir_placeholders_no_slide(slide, dados):
    """Substitui placeholders (chaves do dicionário 'dados') pelo valor correspondente."""
    try:
        for shape in slide.shapes:
            if shape.has_text_frame:
                for paragraph in shape.text_frame.paragraphs:
                    # Precisa iterar nos runs para não perder formatação ao substituir
                    inline_runs = list(paragraph.runs) # Copia para iterar
                    paragraph.clear() # Limpa o parágrafo original
                    current_run = None
                    for run_template in inline_runs:
                        run_text = run_template.text
                        # Aplica todas as substituições no texto do run atual
                        for k, v in dados.items():
                            # Usa a chave diretamente como placeholder
                            placeholder = str(k)
                            if placeholder in run_text:
                                v_str = str(v) if v is not None else ""
                                run_text = run_text.replace(placeholder, v_str)

                        # Adiciona o texto (modificado ou não) a um novo run,
                        # tentando copiar a formatação do run original
                        if run_text: # Só adiciona se houver texto
                            new_run = paragraph.add_run()
                            new_run.text = run_text
                            # Copia formatação básica
                            try:
                                new_run.font.name = run_template.font.name
                                new_run.font.size = run_template.font.size
                                new_run.font.bold = run_template.font.bold
                                new_run.font.italic = run_template.font.italic
                                new_run.font.underline = run_template.font.underline
                                # Copiar cor - requer objeto ColorFormat
                                # if run_template.font.color.rgb:
                                #     new_run.font.color.rgb = run_template.font.color.rgb
                                # else: # Tentar cor do tema
                                #     new_run.font.color.theme_color = run_template.font.color.theme_color
                            except Exception as fmt_err:
                                print(f"Aviso: Erro ao copiar formatação: {fmt_err}") # Não trava
    except Exception as e:
...
# ---------------------------------------------------------
# Classe PlanoFrame (Aba) - Reusa código anterior com ajustes
# ---------------------------------------------------------
class PlanoFrame(ttkb.Frame):
    # __init__ (mantido da versão anterior, com os módulos e variáveis corretas)
    def __init__( self, parent, aba_index, nome_cliente_var_shared, validade_proposta_var_shared, on_close_callback=None):
        ...
        self.valor_anual_editavel = tk.StringVar(value="0.00"); self.desconto_personalizado = tk.StringVar(value="10")
    ...
    def fechar_aba(self): # (Mantido)
        if self.on_close_callback: self.on_close_callback(self.aba_index)

    def on_bling_selected(self, event=None): # (Mantido)
    ...
    def _montar_layout_esquerda(self): # (Layout mantido da versão anterior)
        ...
        # Filtro de módulos para checkboxes (simplificado - mostrar todos não spinbox/sempre fixos)
    ...
    def _montar_layout_direita(self): # (Layout mantido da versão anterior)
        ...
        self.lbl_treinamento = ttkb.Label(frame_valores, text="+ Custo Treinamento: R$ 0,00", font="-size 9"); self.lbl_treinamento.pack(pady=(0, 5), anchor="w", padx=15) # Começa empacotado
    ...
    def on_reset_anual(self): self.user_override_anual_active.set(False); self.atualizar_valores()
    ...
    def on_reset_desconto(self): self.user_override_discount_active.set(False); self.desconto_personalizado.set("10"); self.atualizar_valores() # Reseta para 10% visualmente

    def configurar_plano(self, plano): # (Lógica mantida da versão anterior)
    ...
    def _calcular_extras(self): # (Lógica mantida da versão anterior)
    ...
    def atualizar_valores(self, *args): # (Lógica mantida da versão anterior)
        try: # Adiciona try-except geral para robustez
            ...
            self.computed_desconto_percent = desconto_final_percent; self.computed_custo_adicional = custo_adicional
             ...
             print(f"ERRO em atualizar_valores: {e}")
             traceback.print_exc() # Imprime detalhes do erro no console


    def montar_lista_modulos(self): # (Lógica mantida da versão anterior)
        linhas = []; info = PLAN_INFO.get(self.current_plan, {}); mandatory = info.get("mandatory", [])
        pdv_val = self.spin_pdv_var.get(); if pdv_val > 0: linhas.append(f"{pdv_val}x PDV - Frente de Caixa")
        usr_val = self.spin_users_var.get(); if usr_val > 0: linhas.append(f"{usr_val}x Usuários")
        smart_tef_val = self.spin_smart_tef_var.get()
        if smart_tef_val > 0: linhas.append(f"{smart_tef_val}x Smart TEF"); # if self.current_plan == "Gestão": linhas[-1] += " (Limite: 3)" # Removido limite visual daqui
        for m in mandatory:
            if m not in ["PDV - Frente de Caixa", "Usuários", "Smart TEF"]: linhas.append(f"1x {m}")
        for m, var_m in self.modules.items():
            if m in self.check_buttons and var_m.get() == 1 and m not in mandatory: linhas.append(f"1x {m}")
        unique_mods = []; [unique_mods.append(mod) for mod in linhas if mod not in unique_mods]
        montagem = "\n".join(f"•    {m}" for m in unique_mods); return montagem

    def gerar_dados_proposta(self, nome_closer, cel_closer, email_closer):
            """ Gera o dicionário de dados para preencher o slide da proposta - CHAVES AJUSTADAS """
            ...
            # Combina mensal sem fidelidade com custo adicional se houver
            ...
            if custo_adicional_val > 0.01:
                plano_mensal_display += f" + {custo_adicional_str} ({label_custo})"
            ...
            # Economia
            economia_str = ""; custo_total_mensalizado = (mensal_sem_fid_val * 12) + custo_adicional_val
            custo_total_anualizado = anual_total_val
            if not self.current_plan == "Autoatendimento": # Autoatendimento não tem mensal para comparar
            ...
            # Dicionário final - CHAVES SÃO OS PLACEHOLDERS ESPERADOS NO PPTX
            dados = {
                "montagem_do_plano": montagem,
                "plano_mensal": plano_mensal_display,           # Chave do backup: valor combinado
                "plano_anual": mensal_efetivo_anual_str,       # Chave do backup: valor mensal no anual
                # Adicionar novos placeholders se precisar do anual total, etc.
                "plano_anual_total_pagamento": anual_total_str, # NOVO placeholder sugerido
                "custo_treinamento_valor": custo_adicional_str if custo_adicional_val > 0.01 else "Incluso", # NOVO placeholder sugerido
                "desconto_total": f"{desconto_percent_val}%", # Chave do backup
                "nome_do_plano": nome_plano_final,             # Chave do backup
                "tipo_de_suporte": tipo_suporte,               # Chave do backup
                "horario_de_suporte": horario_suporte,         # Chave do backup
                "validade_proposta": self.validade_proposta_var.get(), # Chave do backup
                "nome_closer": nome_closer,                  # Chave do backup
                "celular_closer": cel_closer,                # Chave do backup
                "email_closer": email_closer,                # Chave do backup
                "nome_cliente": self.nome_cliente_var.get(),   # Chave do backup
                "economia_anual": economia_str                 # Chave do backup
            }
            # print("DEBUG: Dados para PPTX:", dados) # Descomente para depurar
            return dados


# --- Função gerar_material - USANDO MAPEAMENTO DO BACKUP E LÓGICA ATUALIZADA ---
def gerar_material(lista_abas, nome_closer, celular_closer, email_closer):
    ...
    # 1) Coletar Módulos Ativos e Planos Usados (usando nomes ATUAIS do código)
    modulos_ativos_geral = set(); planos_usados_geral = set()
    # Pega dados da primeira aba para placeholders globais (como nome do cliente)
    dados_primeira_aba = lista_abas[0].gerar_dados_proposta(nome_closer, celular_closer, email_closer)

    for aba in lista_abas:
        current_plan_name = aba.current_plan # Nome atualizado do plano (PDV Básico, etc.)
        planos_usados_geral.add(current_plan_name)
        info_aba = PLAN_INFO.get(current_plan_name, {})
        mandatory_aba = info_aba.get("mandatory", [])

        # Adiciona mandatórios (nomes atuais)
        for mod in mandatory_aba: modulos_ativos_geral.add(mod)
        # Adiciona opcionais selecionados (nomes atuais)
        ...
        # Adiciona itens de Spinboxes (nomes atuais)
        ...
        # Adicionar TEF e Terminais se virarem spinbox de novo?
        if aba.modules.get("TEF", tk.IntVar()).get() == 1: modulos_ativos_geral.add("TEF") # Se for checkbox
        if aba.modules.get("Terminais Autoatendimento", tk.IntVar()).get() == 1: modulos_ativos_geral.add("Terminais Autoatendimento") # Se for checkbox

    # print("DEBUG: Módulos Ativos para Material:", modulos_ativos_geral) # Descomente para depurar

    # 2) MAPEAMENTO_MODULOS - Usar placeholders do SEU PPTX (chaves)
    #    e os nomes INTERNOS ATUAIS do Python (valores)
    #    Este mapeamento é crucial e DEVE corresponder ao seu template!
    MAPEAMENTO_MODULOS = {
        # Placeholder no PPTX : Nome INTERNO do Módulo no Python ATUAL
        "slide_sempre": None,
        "check_sistema_kds": "Relatório KDS",
        "check_Hub_de_Delivery": "Hub de Delivery",
        "check_integracao_api": "Integração API",
        "check_integracao_tap": "Integração Tap", # ADDED
        "check_controle_de_mesas": "Controle de Mesas", # UPDATED NAME? Check internal name
        "check_Delivery": "Delivery", # UPDATED NAME? Check internal name
        "check_producao": "Produção", # UPDATED NAME? Check internal name
        "check_Estoque_em_Grade": "Estoque em Grade", # UPDATED NAME? Check internal name
        "check_Facilita_NFE": "Facilita NFE", # UPDATED NAME? Check internal name
        "check_Importacao_de_xml": "Importação de XML", # Case sensitive! Check internal name
        "check_conciliacao_bancaria": "Conciliação Bancária", # UPDATED NAME? Check internal name
        "check_contratos_de_cartoes": "Contratos de cartões e outros", # UPDATED NAME? Check internal name
        "check_ordem_de_servico": "Ordem de Serviço", # ADDED - Check internal name
        "check_relatorio_dinamico": "Relatório Dinâmico", # ADDED - Check internal name
        "check_programa_de_fidelidade": "Programa de Fidelidade", # ADDED
        "check_business_intelligence": "Business Intelligence (BI)", # UPDATED NAME? Check internal name
        "check_smartmenu": "Smart Menu", # ADDED
        "check_backup_real_time": "Backup Realtime", # ADDED
        "check_att_tempo_real": "Atualização em tempo real", # ADDED - Check internal name
        "check_promocao": "Promoções", # ADDED - Check internal name
        "check_marketing": "Marketing", # ADDED - Check internal name
        "pdv_balcao": "PDV - Frente de Caixa", # UPDATED to new internal name
        "qtd_smarttef": "Smart TEF", # Check internal name
        "qtd_tef": "TEF", # Check internal name
        "qtd_autoatendimento": "Terminais Autoatendimento", # UPDATED to new internal name
        "qtd_cardapio_digital": "Cardápio digital", # UPDATED to new internal name
        "qtd_app_gestao_cplug": "App Gestão CPlug", # ADDED - Check internal name
        "check_delivery_direto_vip": "Delivery Direto VIP", # UPDATED NAME? Check internal name
        "check_delivery_direto_profissional": "Delivery Direto Profissional", # UPDATED NAME? Check internal name
        # Placeholder genérico para qualquer tipo de NF ativa
        "check_notas_fiscais": {"Notas Fiscais Ilimitadas", "30 Notas Fiscais"},

        # Placeholders ANTIGOS do backup - Remover se não existirem mais no PPTX
        # "check_..." : "Nome Antigo"
    }

    # 3) Decidir quais slides manter (Lógica do backup)
    keep_slides = set()
    for i, slide in enumerate(prs.slides):
        slide_mantido = False
        try: # Adiciona try-except para evitar erros em slides mal formatados
            for shape in slide.shapes:
                 ...
                 if shape.has_text_frame:
                     for paragraph in shape.text_frame.paragraphs:
                         ...
                         for run in paragraph.runs:
                             ...
                             # Checa placeholders mapeados
                             for placeholder, modulo_mapeado in MAPEAMENTO_MODULOS.items():
                                 if placeholder in txt_run: # Placeholder existe no texto?
                                     if modulo_mapeado is None: slide_mantido = True; break # slide_sempre
                                     elif isinstance(modulo_mapeado, set): # É um conjunto de módulos?
                                         if any(m in modulos_ativos_geral for m in modulo_mapeado): slide_mantido = True; break # Algum ativo?
                                     elif modulo_mapeado in modulos_ativos_geral: slide_mantido = True; break # Módulo único ativo?

                             # Checa condição Bling (se ainda relevante)
                             if "slide_bling" in txt_run and any(p.startswith("Bling -") for p in planos_usados_geral):
                                  slide_mantido = True; break
        except Exception as slide_err:
             print(f"Aviso: Erro ao processar textos do slide {i}: {slide_err}") # Não trava

        if slide_mantido: keep_slides.add(i)

    # 4) Remove slides não mantidos (Lógica do backup)
    if not keep_slides:
         showerror("Erro", "Nenhum slide selecionado para o Material Técnico. Verifique o mapeamento e os módulos ativos.")
         return None

    # print("DEBUG: Slides a manter:", keep_slides) # Descomente para depurar
    slides_removidos = 0
    ...
    print(f"Slides removidos do Material Técnico: {slides_removidos}")

    # 5) Substituir placeholders globais (Lógica do backup, usando dados da primeira aba)
    for slide in prs.slides:
        substituir_placeholders_no_slide(slide, dados_primeira_aba) # Usa a função simples

    # 6) Salvar pptx final (Lógica do backup)
...
# (Omitido para brevidade - igual à versão anterior)
SCOPES = ['https://www.googleapis.com/auth/drive']
def baixar_client_secret_remoto(): # ... (código igual)
    ...
    if not os.path.exists(nome_local): print("Baixando client_secret..."); r = requests.get(url, timeout=15); r.raise_for_status(); open(nome_local, "w", encoding="utf-8").write(r.text)
    return nome_local
def get_gdrive_service(): # ... (código igual)
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
             ...
             except Exception as e: showerror("Erro de Autenticação", f"Falha: {e}"); return None
    ...
    except Exception as e: showerror("Erro Google API", f"Falha: {e}"); return None
def upload_pptx_and_export_to_pdf(local_pptx_path): # ... (código igual)
    ...
    try:
        ...
        if not file_id: raise Exception("Upload falhou (sem ID)."); print(f"Upload concluído. ID: {file_id}. Exportando PDF...")
    ...
    except Exception as e: showerror("Erro Google Drive", f"Erro:\n{e}")
    finally:
        if file_id:
...
class MainApp(ttkb.Window): # ... (código igual à versão anterior)
    def __init__(self):
        super().__init__(themename="litera"); self.title("Gerador de Propostas ConnectPlug v2.2"); self.geometry("1200x800")
    ...
    def on_gerar_tudo(self):
        ...
        pptx_prop = gerar_proposta(abas, self.nome_closer_var.get(), self.celular_closer_var.get(), self.email_closer_var.get())
        if pptx_prop and os.path.exists(pptx_prop): try: upload_pptx_and_export_to_pdf(pptx_prop); pdf_prop_ok = True; except Exception as e: print(f"Erro PDF Proposta: {e}")
        print("--- Gerando Material Técnico ---"); pdf_mat_ok = False
        pptx_mat = gerar_material(abas, self.nome_closer_var.get(), self.celular_closer_var.get(), self.email_closer_var.get())
        if pptx_mat and os.path.exists(pptx_mat): try: upload_pptx_and_export_to_pdf(pptx_mat); pdf_mat_ok = True; except Exception as e: print(f"Erro PDF Material: {e}")
        print(f"--- Geração Concluída ({'OK' if pdf_prop_ok else 'Erro PDF'} Proposta, {'OK' if pdf_mat_ok else 'Erro PDF'} Material) ---")
